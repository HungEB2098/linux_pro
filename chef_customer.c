...
int main()
{
    ...
    sem_init(mutex1, 1, 1); //khóa mutex
    sem_init(empty1, 1, BUFFER_SIZE); //--> đầy

    sem_init(full1, 1, 0);//-> trống
...
//    while(1) {
    ...
    //         printf("Dish in tray non-vegan buffer: %d/%d\n", takenSlots, BUFFER_SIZE);
    //     }
    // }

            /* Child customer vegan process */
    if((customer2 = fork()) == 0)
    {
        while(1)
        {
    ...
    //         sleep(10);
    //         int takenSlots;
    //         sem_getvalue(full1, &takenSlots);
    //         printf("Dish in tray vegan buffer: %d/%d\n", takenSlots, BUFFER_SIZE);
    //     }
    // }
            /* Child customer all process */
    if((customer3 = fork()) == 0)
    {
        while(1)
        {
    ...
    // waitpid(chef1, &status, 0);
    // waitpid(chef2, &status, 0);
    // waitpid(customer1, &status, 0);
    // waitpid(customer2, &status, 0);
    // waitpid(customer3, &status, 0);
